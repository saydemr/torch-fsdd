...
def test_generator_repo_master():
    path = "src/test/data/recordings"
    try:
        fsdd = TorchFSDDGenerator(version="master", path="src/test/data")
...
def test_generator_repo_tag1():
    path = "src/test/data/recordings"
    try:
        fsdd = TorchFSDDGenerator(version="v1.0.3", path="src/test/data")
        with open("src/test/filenames/v1.0.3") as f:
            true_files = [l.rstrip() for l in f.readlines()]
        read_files = [os.path.basename(file) for file in fsdd.all_files]
        assert len(true_files) == len(read_files) and sorted(true_files) == sorted(
            read_files
...
def test_generator_repo_tag2():
    path = "src/test/data/recordings"
    try:
        fsdd = TorchFSDDGenerator(version="v1.0.10", path="src/test/data")
        with open("src/test/filenames/v1.0.10") as f:
            true_files = [l.rstrip() for l in f.readlines()]
        read_files = [os.path.basename(file) for file in fsdd.all_files]
        assert len(true_files) == len(read_files) and sorted(true_files) == sorted(
...
def test_generator_train_test_split_99_1():
    fsdd = TorchFSDDGenerator(version="local", path="src/test/data/v1.0.10")
...
def test_dataset_no_transforms():
    fsdd = TorchFSDD(glob.glob("src/test/data/v1.0.10/*.wav"))
    x, y = fsdd[0]
...
def test_dataset_transforms_single():
    files = glob.glob("src/test/data/v1.0.10/*.wav")
...
def test_dataset_transforms_multiple():
    ...
    x_trans, _ = TorchFSDD(
        files,
        transforms=Compose(
            [TrimSilence(threshold=0.05), MFCC(sample_rate=8e3, n_mfcc=n_mfcc)]
        ),
    )[0]
    assert isinstance(x_trans, torch.Tensor)
    assert x_trans.ndim == 2
    assert len(x_trans) == n_mfcc
    # assert y == 7
